...
5. 启动驱动节点：
   ...
   - 确认当前IM1R连接的串口，如`/dev/ttyUSB0`
   - 确认当前IM1R使用的波特率，如`115200`
...
9. (可选)TF配置：
   ...
   - 坐标系ID: ds_imu_link
   - 配置说明：如需可视化，请根据实际安装方式自行发布静态TF变换

## 发布的话题

- `imu/data` ([sensor_msgs/Imu](http://docs.ros.org/api/sensor_msgs/html/msg/Imu.html)) 四元数、角速度和线性加速度
- `imu/rawdata` ([sensor_msgs/Imu](http://docs.ros.org/api/sensor_msgs/html/msg/Imu.html)) 未经标定的原始角速度和线性加速度
